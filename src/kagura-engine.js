...
function aggregateTaskResults(taskResults) {
  const facts={
    ...
    unconfirmed:taskResults.flatMap((r)=>(r.facts.unconfirmed||[]).map((x)=>({task_id:r.task.id,...x}))),
    opinions:taskResults.flatMap((r)=>(r.facts.opinions||[]).map((x)=>({task_id:r.task.id,...x}))),
    not_applicable:taskResults.flatMap((r)=>(r.facts.not_applicable||[]).map((x)=>({task_id:r.task.id,...x}))),
    corroborating_evidence:taskResults.flatMap((r)=>(r.facts.corroborating_evidence||[]).map((x)=>({task_id:r.task.id,...x}))),
    evidence_need:taskResults.flatMap((r)=>(r.facts.evidence_need||[]).map((x)=>({task_id:r.task.id,...x}))),
    evidence_gaps:taskResults.flatMap((r)=>(r.facts.evidence_gaps||r.facts.evidence_need||[]).map((x)=>({task_id:r.task.id,...x}))),
  ...
  const inquiry={ pillar:'inquiry',problem_health:{healthy:taskResults.every((r)=>r.inquiry.problem_health?.healthy),reason:join(taskResults.filter((r)=>!r.inquiry.problem_health?.healthy).map((r)=>`${r.task.id}:${r.inquiry.problem_health?.reason}`),'全Taskの前提条件は追跡可能')},missing_fields:unique(taskResults.flatMap((r)=>(r.inquiry.missing_fields||[]).map((x)=>`${r.task.id}:${x}`))),missing_questions:unique(taskResults.flatMap((r)=>r.inquiry.missing_questions||[])),inquiry_lens:unique(taskResults.flatMap((r)=>r.inquiry.inquiry_lens||[])),per_task:Object.fromEntries(taskResults.map((r)=>[r.task.id,r.inquiry])) };
  const multi={ pillar:'multi',per_task:Object.fromEntries(taskResults.map((r)=>[r.task.id,r.multi])),perspectives:taskResults.flatMap((r)=>(r.multi.perspectives||[]).map((x)=>({task_id:r.task.id,...x}))),angles:taskResults.length===1?(taskResults[0].multi.angles||{}):{per_task:Object.fromEntries(taskResults.map((r)=>[r.task.id,r.multi.angles||{}]))},trade_off_map:taskResults.flatMap((r)=>(r.multi.trade_off_map||[]).map((x)=>({task_id:r.task.id,...x}))) };
  const dialectic={ pillar:'dialectic',engine:'Astera Deterministic Dialectic',mode:'decision_materials',per_task:Object.fromEntries(taskResults.map((r)=>[r.task.id,r.dialectic])),candidates:taskResults.flatMap((r)=>(r.dialectic.candidates||[]).map((x)=>({task_id:r.task.id,...x}))),bad_hand_lessons:unique(taskResults.flatMap((r)=>r.dialectic.bad_hand_lessons||[])) };
...
function buildHyperionCompatibility(dialectic, comparison) {
  const rankingByKey = new Map((comparison.candidate_ranking || []).map((candidate) => [`${candidate.task_id}:${candidate.id}`, candidate]));
  return {
    ...dialectic,
    compatibility_contract: {
      schema_version: 'astera.hyperion.dialectic.compat.v1',
      score_source: 'compare.candidate_ranking',
      decision_authority: 'compare'
    },
    candidates: (dialectic.candidates || []).map((candidate) => {
      const ranked = rankingByKey.get(`${candidate.task_id}:${candidate.id}`);
      return {
        ...candidate,
        score: Number.isInteger(ranked?.score) ? ranked.score : null,
        answer_line_distance: Number.isInteger(ranked?.answer_line_distance) ? ranked.answer_line_distance : null
      };
    })
  };
}
...
class KaguraEngine {
  ...
  async process(input={}, tenant={id:'unknown'}) {
    ...
    const shortOpaqueDegraded = request.instruction_understanding?.mode === 'DEGRADED' && packet.tasks.length === 1 && packet.tasks[0].id === 'JP-MCP-REQUIRED' && Array.from(question).length <= 4;
    if (shortOpaqueDegraded) {
      const questions=[lang==='ja'?'判断対象・目的・完了条件を具体化してください。':'Please specify the target, objective, and completion condition.'];
      return {result:{type:'clarification_needed',request_model:request,questions},material:this.clarify(questions,lang),prompt:'',runtime:{ai_used:false,llm_called:false,engine:'v8_deterministic_rules',instruction_mode:'DEGRADED',task_count:1,wave_count:1}};
    }
    ...
    const compatibilityDomain=taskResults.length===1?taskResults[0].task.domain:{taxonomy_version:taskResults[0]?.task.domain?.taxonomy_version||null,primary:null,secondary:[],overlays:[],per_task:Object.fromEntries(taskResults.map((r)=>[r.task.id,r.task.domain]))};
    const compatibilityDialectic=buildHyperionCompatibility(aggregate.dialectic, aggregate.comparison);
    const result={type:'cognitive_map',mode:'deterministic_multi_parallel_decision_materials',non_ai:true,request_model:request,analysis_task_packet:request.analysis_task_packet,five_stage:fiveStage,task_results:taskResults.map((r)=>({task:r.task,evidence:r.evidence,preflight:r.preflight,facts:r.facts,risks:r.risks,inquiry:r.inquiry,multi:r.multi,dialectic:r.dialectic,comparison:r.comparison})),domain:compatibilityDomain,facts:aggregate.facts,risks:aggregate.risks,multi:aggregate.multi,inquiry:aggregate.inquiry,hyperion:{engine:'Astera Deterministic Dialectic',mode:'decision_materials',dialectic:compatibilityDialectic},comparison:aggregate.comparison,judgment};
  ...
  frame({request,context,taskResults,aggregate,lang}) {
    ...
    const singleDomain=taskResults.length===1?taskResults[0].task.domain:null;
    const domainTemplate=singleDomain?{primary:singleDomain.primary||null,secondary:singleDomain.secondary||[],overlays:singleDomain.overlays||[],taxonomy_version:singleDomain.taxonomy_version||singleDomain.primary?.taxonomy_version||null}:{primary:null,secondary:[],overlays:[],taxonomy_version:taskResults[0]?.task.domain?.taxonomy_version||null,per_task:Object.fromEntries(taskResults.map((r)=>[r.task.id,r.task.domain]))};
    ...
    const domainCompareLens=unique(taskResults.flatMap((r)=>r.task.domain.primary?.compare_lens||[]));
    ...
    const sections={
      ...
      '03_facts':{summary:lang==='en'?`supported=${aggregate.facts.confirmed.length}, unresolved=${aggregate.facts.unconfirmed.length}, opinions=${aggregate.facts.opinions.length}`:`根拠支持${aggregate.facts.confirmed.length}件 / 未確認${aggregate.facts.unconfirmed.length}件 / 意見${aggregate.facts.opinions.length}件`,items:factItems,confirmed:aggregate.facts.confirmed,unconfirmed:aggregate.facts.unconfirmed,opinions:aggregate.facts.opinions,evidence_to_collect:evidenceToCollect,decision_basis:trace(['MAIN8-03-EVIDENCE-GATED-FACT'],{derivation:'Evidence Gate通過命題だけSUPPORTED、それ以外は未確認/意見に保持する。'})},
      '04_crisis':{summary:aggregate.risks.highest?`${aggregate.risks.level}: ${aggregate.risks.highest.task_id}:${aggregate.risks.highest.impact}`:(lang==='en'?'No material risk detected.':'重大Riskは検出されていない。'),items:riskItems,highest:aggregate.risks.highest,risks:aggregate.risks.risks,domain_checks:aggregate.risks.domain_checks||[],decision_basis:trace(['MAIN8-04-RISK-WEIGHT-HARD-GATE'],{derivation:'Task別Risk Rule・Parser Guard・禁止条件をHard Gateとして統合する。'})},
      '05_opposition':{summary:join(oppositionItems),items:oppositionItems,domain_perspectives:domainPerspectives,decision_basis:trace(['MAIN8-05-DIALECTIC-OPPOSITION'],{derivation:'各Taskで同一Metricから生成したopposition候補を保持する。'})},
      '06_comparison':{summary:`global=${aggregate.comparison.score} / decision=${aggregate.comparison.verdict.decision} / ${join(comparisonItems)}`,items:comparisonItems,selected_candidate:aggregate.comparison.selected_candidate,candidate_ranking:aggregate.comparison.candidate_ranking,rejected_candidates:aggregate.comparison.rejected_candidates,domain_compare_lens:domainCompareLens,decision_basis:trace(['MAIN8-06-WEIGHTED-CANDIDATE-COMPARE','MAIN8-06-MIN-TASK-BOTTLENECK'],{derivation:'Task内は同一5Metric加重Score、Task間は最弱TaskをBottleneckとして弱点を平均で隠さない。'})},
    ...
    return {format:'astera_judgment_v4',canonical_language:'en',output_language:lang,order:[...ORDER],non_ai:true,instruction_understanding:request.instruction_understanding||null,domain_template:domainTemplate,task_graph:{task_count:taskResults.length,dependencies:packet.dependencies,execution_waves:packet.execution_waves,hard_blockers:packet.hard_blockers||[]},lens_routing:{per_task:Object.fromEntries(taskResults.map((r)=>[r.task.id,{primary:r.task.domain.primary||null,secondary:r.task.domain.secondary||[],overlays:r.task.domain.overlays||[],classification_basis:r.task.domain.classification_basis,confidence:r.task.domain.confidence}]))},evidence_state:{per_task:Object.fromEntries(taskResults.map((r)=>[r.task.id,{required:r.task.evidence_need.required,state:r.evidence.state,source_status:r.evidence.source_status,quality_score_bp:r.evidence.quality_score_bp,quality_gate_bp:r.evidence.quality_gate_bp,coverage_state:r.evidence.coverage_state,conflict_detected:r.evidence.conflict_detected,authorities:r.evidence.distinct_authority_count||0,source_families:r.evidence.distinct_source_family_count||0,reinforcement:r.evidence.reinforcement_attempt_count||0,corroboration:r.evidence.new_corroboration_count||0}]))},...sections};
  }

  material(judgment) {
    const sections=ORDER.map((k)=>judgment[k]); const compact_text=sections.map((s)=>`${s.label}: ${line(s.summary)}`).join('\n'); const heads=judgment.output_language==='en'?{one:'Decision Material',basis:'Decision Basis',pass:'Material for External Consumer'}:{one:'一言説明 / 判断材料',basis:'判断基準',pass:'主役AIへ渡す内容'};
