...
function taskRouteText(task) { return unique([...(task.premises || []), task.source_span?.text || '']).join('\n'); }
...
class KaguraEngine {
  ...
  async process(input={}, tenant={id:'unknown'}) {
    ...
    const fiveStage={schema_version:'astera.five-stage.v1',order:[...FIVE_STAGE],execution_waves:request.analysis_task_packet.execution_waves,tasks:taskResults.map((r)=>({task_id:r.task.id,lens_id:r.task.domain.primary?.id||null,evidence_state:r.evidence.state,fact:r.facts,risk:r.risks,multi:r.multi,inquiry:r.inquiry,compare:r.comparison,dialectic:r.dialectic}))};
  ...
  frame({request,context,taskResults,aggregate,lang}) {
    ...
    const sections={
      ...
      '08_reinstruction':{summary:join(reinstructionItems),items:reinstructionItems,decision_basis:trace(['MAIN8-08-LOSSLESS-REINSTRUCTION'],{derivation:'Task順序・禁止・維持・Parser状態・Evidence状態・未確認・Blocking条件を失わず外部Consumerへ再指示する。'})}
    };
    ORDER.forEach((key,i)=>{sections[key]={canonical_label:CANON[i],label:labels[i],...sections[key]};});
    return {format:'astera_judgment_v4',canonical_language:'en',output_language:lang,order:[...ORDER],non_ai:true,instruction_understanding:request.instruction_understanding||null,task_graph:{task_count:taskResults.length,dependencies:packet.dependencies,execution_waves:packet.execution_waves,hard_blockers:packet.hard_blockers||[]},lens_routing:{per_task:Object.fromEntries(taskResults.map((r)=>[r.task.id,{primary:r.task.domain.primary||null,secondary:r.task.domain.secondary||[],overlays:r.task.domain.overlays||[],classification_basis:r.task.domain.classification_basis,confidence:r.task.domain.confidence}]))},evidence_state:{per_task:Object.fromEntries(taskResults.map((r)=>[r.task.id,{required:r.task.evidence_need.required,state:r.evidence.state,source_status:r.evidence.source_status,quality_score_bp:r.evidence.quality_score_bp,coverage_state:r.evidence.coverage_state,conflict_detected:r.evidence.conflict_detected}]))},...sections};
  }
