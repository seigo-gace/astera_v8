...
function buildEvidenceRegistryFromSearchResult(searchResult = {}) {
  const candidates = Array.isArray(searchResult.evidence) ? searchResult.evidence : [];
  const queryExecution = searchResult.query_execution && typeof searchResult.query_execution === 'object'
    ? searchResult.query_execution
    : {};
  if (candidates.length === 0) {
    return deepFreeze({ registry: emptyEvidenceRegistry(), bindings: emptyEvidenceBindings() });
  }
  const requestId = String(searchResult.request_id || '');
  const bindings = buildBindings({ candidates, queryExecution, requestId });
  const entries = candidates
    .map((candidate) => buildEntry(candidate, bindings, {
      requestId,
      queryPlanHash: searchResult.query_plan_hash || '',
      executionTime: searchResult.execution_time || null,
      effectiveAsOf: searchResult.effective_as_of || null
    }))
...
module.exports = {
  ...
  buildEvidenceRegistryFromSearchResult,
