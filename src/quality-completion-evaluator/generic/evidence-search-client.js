...
class EvaluatorEvidenceSearchClient {
  constructor(options = {}) {
    this.baseUrl = String(
      options.baseUrl
    ...
    if (typeof this.fetch !== 'function') throw new Error('EvaluatorEvidenceSearchClient requires fetch');
  }
  ...
  async search(payload, context = {}) {
    ...
    try {
      const callerId = String(context.callerId || '').trim();
      const requestId = String(context.requestId || '').trim();
      if (!callerId || !requestId) {
        const error = new Error('evidence search callerId and requestId are required');
        error.code = 'EVIDENCE_API_CONTEXT_REQUIRED';
        throw error;
      }
      ...
      const response = await this.fetch(`${this.baseUrl}/internal/v1/evidence/search`, {
        method: 'POST',
      ...
      if (error?.name === 'AbortError') {
        if (externallyCancelled || externalSignal?.aborted) {
          const cancelled = new Error('evidence search API cancelled by evaluator');
...
module.exports = EvaluatorEvidenceSearchClient;
