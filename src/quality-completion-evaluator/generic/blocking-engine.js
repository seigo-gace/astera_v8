...
function evaluateGenericBlocking(profile, metricEvaluation, evidenceVerification) {
  ...
  const invalidRefIssues = metricEvaluation.issues.filter((item) => item.code === "METRIC_EVIDENCE_REF_INVALID" || item.code === "METRIC_EVIDENCE_CLAIM_UNRESOLVED");
  if (invalidRefIssues.length > 0) {
    blocks.push(block("EVAL-HB-002", "invalid_evidence_reference", "Measurement references unknown Evidence/Binding IDs or unresolved evidence claims", [], { issues: invalidRefIssues }));
  }
  ...
  const integrityCodes = new Set([
    ...
    "MEASUREMENT_EVIDENCE_CLAIM_REF_DUPLICATE",
  ...
  for (const rule of Array.isArray(profile.hard_blocks) ? profile.hard_blocks : []) {
    ...
    const refs = metricEvaluation.effective_refs_by_metric.get(rule.metric_id) || [];
