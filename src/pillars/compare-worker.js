...
function detectContradictions({ facts = {}, risks = {}, inquiry = {}, evidence, ranking = [], task = {} }) {
  ...
  if (evidence.state === 'REJECTED') contradictions.push({ key: 'evidence_rejected', note: `Evidence Search=${evidence.source_status}。根拠確定として扱えない。` });
...
async function run({ question = '', facts = {}, risks = {}, multi = {}, inquiry = {}, dialectic = {}, domain = {}, task = null, evidence_packet = null, dependency_state = {} }) {
  const t = task || { id: null, target: '対象', objective: question, action: 'analyze', constraints: [], prohibitions: [], preserve: [], replace: [], depends_on: [] };
  ...
  if (score >= 60 && (score < 80 || contradictions.length || (t.evidence_need?.required && evidence.state !== 'VALID'))) { decision = 'recommend_with_caution'; gates.push('COMPARE-GATE-CAUTION'); }
  if (t.evidence_need?.required && evidence.state === 'REJECTED' && (facts.confirmed?.length || 0) === 0) { decision = 'hold_and_clarify'; gates.push('COMPARE-GATE-EVIDENCE-REJECTED'); }
  ...
  const rationale = selected
    ? `${selected.label}をTask ${t.id || '-'}の首位とする。目的適合=${selected.metrics.objectiveFit ?? '-'}、Evidence適合=${selected.metrics.evidenceFit ?? '-'}、Risk制御=${selected.metrics.riskControl ?? '-'}、Constraint適合=${selected.metrics.constraintFit ?? '-'}、Rollback性=${selected.metrics.reversibility ?? '-'}。${runnerUp ? `次点${runnerUp.label}との差=${margin}。` : ''}`
    : '比較可能な候補がない。';
  ...
  return {
    ...
    contradictions, dependency_blockers: dependencyBlockers, selected_candidate: selected, candidate_ranking: ranking,
    ...
    uncertainty: { evidence_state: evidence.state, evidence_quality_score_bp: evidence.quality_score_bp, unresolved_fact_count: facts.unconfirmed?.length || 0, missing_field_count: inquiry.missing_fields?.length || 0, selection_margin: margin },
