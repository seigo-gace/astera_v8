...
function classifyAssertionType(text){const value=normalizeText(text);if(containsAny(value,ASSERTION_TYPE_SEED.question))return AssertionType.QUESTION;if(POLARITY_SEED.doubleNegative.some((token)=>value.includes(token)))return AssertionType.ASSERTION;if(COMPLEX_NEGATION_PATTERNS.some((pattern)=>value.includes(pattern)))return AssertionType.ASSERTION;if(MODALITY_SEED.conditional.some((token)=>value.includes(token)))return AssertionType.ASSERTION;if(containsAny(value,ASSERTION_TYPE_SEED.command))return AssertionType.COMMAND;if(containsAny(value,ASSERTION_TYPE_SEED.wish))return AssertionType.WISH;if(containsAny(value,ASSERTION_TYPE_SEED.evaluation))return AssertionType.EVALUATION;if(containsAny(value,ASSERTION_TYPE_SEED.assertion))return AssertionType.ASSERTION;return AssertionType.UNKNOWN;}
