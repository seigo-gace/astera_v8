...
const ASSERTION_TYPE_SEED=Object.freeze({
  ...
  wish:Object.freeze(['したい','してほしい','望む','欲しい','ほしい','が欲しい','を欲しい','改善したい','を実現したい','にしたい']),
  evaluation:Object.freeze(['と思う','と考える','べき','かもしれない','だろう','はず','気がする']),
  assertion:Object.freeze(['である','です','した','している','できる','ある','いる'])
});
...
function isOutputPolicyFragment(normalized){return/(?:最終結論|最終判断)(?:は|を)?(?:出さ|しない|禁止)|判断材料(?:だけ|のみ|のみを)?(?:欲しい|ほしい|が欲しい)|材料(?:だけ|のみ)(?:欲しい|ほしい|が欲しい|で)/u.test(normalized);}
function isUserConstraintFragment(normalized){return/(?:予算|コスト|費用)(?:は|:)?\s*\d+(?:\.\d+)?%|(?:納期|期限|締切|デッドライン)(?:は|:)?|(?:既存|現在)の?(?:ユーザー|利用者|操作)(?:は|を)?(?:変え(?:ない|ず)|維持)/u.test(normalized);}
function parseNaturalStructure(text){const normalized=stripEnding(text);if(COMPLEX_NEGATION_PATTERNS.some((pattern)=>normalized.includes(pattern)))return{unresolved:true,diagnostic:'COMPLEX_NEGATION'};if(normalized==='動かないことはない')return{subject:'動作',predicate:'POSSIBLE',object_or_value:true};if(isOutputPolicyFragment(normalized)){const parts=[];if(/(?:最終結論|最終判断)(?:は|を)?(?:出さ|しない|禁止)/u.test(normalized))parts.push('FINAL_DECISION_PROHIBITED');if(/判断材料|材料(?:だけ|のみ)/u.test(normalized))parts.push('MATERIAL_ONLY_REQUIRED');return{subject:'OUTPUT',predicate:'OUTPUT_POLICY',object_or_value:parts.length?parts.join('|'):normalized,predicate_parts:Object.freeze(parts)};}if(isUserConstraintFragment(normalized)){if(/(?:既存|現在).{0,12}(?:操作|ユーザー|利用者).{0,12}(?:変え(?:ない|ず)|維持)/u.test(normalized))return{subject:'USER_OPERATION',predicate:'PRESERVE',object_or_value:normalizeText(normalized)};if(/(?:納期|期限|締切|デッドライン)/u.test(normalized))return{subject:'DEADLINE',predicate:'USER_DEADLINE',object_or_value:normalizeText(normalized)};return{subject:'CONSTRAINT',predicate:'USER_CONSTRAINT',object_or_value:normalizeText(normalized)};}if(/(?:改善したい|を改善したい|してほしい|が欲しい|を欲しい|したい)$/u.test(normalized)){const goal=normalized.replace(/(?:が|を)?(?:欲しい|ほしい)$/u,'').replace(/したい$/u,'する');return{subject:'USER',predicate:'USER_GOAL',object_or_value:normalizeText(goal||normalized)};}const conditional=parseConditional(normalized);if(conditional)return conditional;if(/リリースされる$/u.test(normalized))return{subject:'リリース',predicate:'OCCURS',object_or_value:true};const japanese=normalized.match(/^(.+?)(?:は|が)(.+)$/u);if(japanese&&!/(?:欲しい|ほしい|したい|してほしい|改善)/u.test(normalized))return{subject:normalizeText(japanese[1]),predicate:'HAS_STATE',object_or_value:normalizeText(japanese[2])};const english=normalized.match(/^([A-Za-z0-9_.:/ -]+)\s+(?:is|are|was|were)\s+(.+)$/iu);if(english)return{subject:normalizeText(english[1]),predicate:'HAS_STATE',object_or_value:normalizeText(english[2])};const numeric=normalized.match(/^(.+?)\s+([A-Za-z_][A-Za-z0-9_.-]*)\s+(\d+(?:\.\d+)?(?:円|GB|%|％|件|ms|req)?)$/u);if(numeric)return{subject:normalizeText(numeric[1]),predicate:normalizeText(numeric[2]),object_or_value:normalizeText(numeric[3])};return{unresolved:true,diagnostic:'SUBJECT_PREDICATE_OBJECT_UNRESOLVED'};}
