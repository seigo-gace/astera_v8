...
const CANDIDATES = `- Colibrì: GLM-5.2 744Bを25GB RAMのPCで動かす純C製エンジン。
...
const POST_IMPLICIT = `低スペックPCで大規模AIを動かす候補。\n${CANDIDATES}`;
...
function assertStandaloneReviewPrepared(prepared) {
  ...
  assert.ok(prepared.analysis_task_packet.tasks.some((task) => task.evidence_need?.required === true));
}

function assertStandaloneReviewMain8(out) {
  ...
  assert.ok(out.result.analysis_task_packet.tasks.some((task) => task.evidence_need?.required === true));
...
test('public AsteraEngine prepareRequest resolves explicit standalone review material from question only', async () => {
  const engine = createEngine();
  try {
    const prepared = await engine.prepareRequest({ question: POST });
    assertStandaloneReviewPrepared(prepared);
  } finally {
    await engine.destroy();
  }
});

test('public AsteraEngine process produces explicit standalone review Main8 from question only', async () => {
  const engine = createEngine();
  try {
    const out = await engine.process({ question: POST }, { id: 'standalone-main8-explicit' });
    assertStandaloneReviewMain8(out);
  } finally {
    await engine.destroy();
  }
});

test('public AsteraEngine auto-detects review from material alone with no purpose instruction', async () => {
  const engine = createEngine();
  try {
    const prepared = await engine.prepareRequest({ question: POST_IMPLICIT });
    assertStandaloneReviewPrepared(prepared);
    assert.equal(prepared.analysis_task_packet.analysis_intent.source, 'OBSERVABLE_MATERIAL');

    const out = await engine.process({ question: POST_IMPLICIT }, { id: 'standalone-main8-implicit' });
    assertStandaloneReviewMain8(out);
    assert.equal(out.result.judgment.analysis_intent.source, 'OBSERVABLE_MATERIAL');
  } finally {
    await engine.destroy();
  }
});
