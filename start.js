...
const store = new SQLiteStore(process.env.ASTERA_DB || 'astera.db');
const logger = new Logger();
const server = new AsteraServer({
  port: Number(process.env.ASTERA_PORT || 7373),
  host: process.env.ASTERA_HOST || '127.0.0.1',
  poolSize: Number(process.env.ASTERA_POOL || 4),
  store,
  logger
});
...
process.on('uncaughtException', (error) => { logger.write({ type: 'uncaught_exception', severity: 'error', text: 'Uncaught exception', payload: { error } }); void shutdown('uncaughtException', 1); });
process.on('unhandledRejection', (reason) => { logger.write({ type: 'unhandled_rejection', severity: 'error', text: 'Unhandled promise rejection', payload: { reason } }); void shutdown('unhandledRejection', 1); });
process.on('warning', (warning) => { logger.write({ type: 'runtime_warning', severity: 'warn', text: warning.message, payload: { warning } }); });
