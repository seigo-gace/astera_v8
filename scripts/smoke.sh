...
SMOKE_API_KEY="${key}" node -e "const port=process.env.ASTERA_PORT; const key=process.env.SMOKE_API_KEY; const body={question:'マーケティング施策を決めたい。対象は小規模SaaSの見込み客。CVは無料登録。広告コピーとLP訴求を比較したい。',language:'ja',llm:{chain:['null']}}; fetch('http://127.0.0.1:'+port+'/process',{method:'POST',headers:{'Content-Type':'application/json','X-API-Key':key},body:JSON.stringify(body)}).then(async r=>{const s=await r.text(); if(!r.ok) { console.error(s); process.exit(1); } const checks=[['Main8 start',/01 本当の目的/],['Main8 end',/08 主役AIへの再指示/],['G10 lens trace',/lenses=G10/],['evidence trace',/evidence_refs=/],['decision trace rules',/rules=/]]; for (const [name,re] of checks) { if(!re.test(s)) { console.error('smoke missing: '+name+'\n'+s); process.exit(1); } } console.log('smoke ok: G10 lens trace, evidence trace, and 8-section judgment output');}).catch((err)=>{console.error(err.message); process.exit(1);});"
